...
export default function SensorCard({ title, id, data, adjustAqi = 0 }) {
  if (!data) return null;
  ...
  const aqi = calculatePM25AQI(pm25) + adjustAqi;
  ...
  return (
    <div className="bg-white rounded-2xl p-8 shadow w-[420px] max-w-full">
      ...
      <div className="flex justify-between items-start gap-4">
        ...
        <span className={"px-4 py-1.5 rounded-full text-xs font-medium whitespace-nowrap " + status.color}>
          {status.label}
        </span>
      </div>
      ...
      <div className="text-center my-8">
        ...
        <p className="text-xs tracking-widest text-gray-500 mt-1">AIR QUALITY INDEX</p>
      </div>
